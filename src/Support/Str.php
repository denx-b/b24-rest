...
class Str
{
    ...
    public static function extractThirdLevelDomain(string $url): string
    {
        ...
        // Минимум: third.second.tld → 3 сегмента
        if (count($segments) < 3) {
            return '';
        }
    ...
    public static function formatIfDate($input)
    {
        if (!is_string($input) || self::filterString($input) === '') {
            return $input;
        }

        $input = self::filterString($input);
